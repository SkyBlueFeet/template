...
import _assignModal from '@core/layout/components/_assign-modal.ejs';
import _modalContent from '@core/layout/snippets/_modal.ejs';
import { roleTableConfig, roleFormConfig } from '@core/config';

let tableHtml = _table({ config: roleTableConfig });
export default layout.initArgon({ main: content({ table: tableHtml }), modal: _modal(_modalContent({ config: roleFormConfig({}, []) })) + _assignModal([]) });