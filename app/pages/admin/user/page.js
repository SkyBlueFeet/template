...
import _assignRole from 'layout/components/_assign-modal.ejs';
import { roleFormConfig, roleTableConfig } from 'app/config';

let tableHtml = _table({ config: roleTableConfig });
export default layout.initArgon({ main: content({ table: tableHtml }), modal: _modal({ config: roleFormConfig({}, []) }) + _assignRole() });