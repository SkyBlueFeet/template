...
import application, { roleRes } from 'static/application';

import { user, auth } from 'app/static/apis';
import _modal from 'layout/snippets/_modal.ejs';
import _assignRole from 'layout/snippets/_assign-Role.ejs';
...
application.run(function(that) {
    $(function() {
        ...
        $(document).on('show.bs.modal', '#adminModal', function(event) {
            ...
            let tableQuery = {};
            const moduleData = that.getRes('user');
        ...
        $('#assignModal').on('show.bs.modal', function(params) {
            ...
            $(this).find('.modal-body').html(_assignRole(that.getRes('role')));
